...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
			...
			'upload_dir' => 'uploads/albums/'.$artistuniqueid.'/'.$albumsessionid.'/',
            'upload_url' => 'uploads/albums/'.$artistuniqueid.'/'.$albumsessionid.'/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            'readfile_chunk_size' => 10 * 1024 * 1024 * 1024 * 1024, // 10 MiB
            // Defines which files can be displayed inline when downloaded:
            'inline_file_types' => '/\.(ogg|wav|mp3)$/i',
            // Defines which files (based on their names) are accepted for upload:
            'accept_file_types' => '/.(ogg|wav|mp3)$/i',
    ...
    protected function trim_file_name($name,
        ...
        if (strpos($name, '.') === false &&
            preg_match('/^audio\/(mp3|mp4)/', $type, $matches)) {
            $name .= '.'.$matches[1];
        }
        return $name;
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        //Call encodeSpecialCharacter() for remove special characters from file name
		$name = encodeSpecialCharacter($name);
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
					...
					$_SESSION['trackfiles'][]	=	array($name,$size);
                }
            } else {
